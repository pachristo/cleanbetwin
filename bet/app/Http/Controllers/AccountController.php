...
//            'phone' => 'required|string',
        ...
        if ($valEmail->fails()) JSONResponder::validationMessage('EMAIL ALREADY IN USE!');

        $valPassword = Validator::make($data, [
            'password' => 'required|confirmed'
        ]);
        if ($valPassword->fails()) JSONResponder::validationMessage('PASSWORD DO NOT MATCH');
    ...
    public function postLogin(Request $request, Sentinel $sentinel, User $user)
    {
        ...
        try {
            if ($sentinel->authenticate(['login' => $email, 'password' => $password], $rememberme))
            {
                $user = currentUser();
                ...
                $redirect = Session::get('loginRedirect', route('/app/index'));
