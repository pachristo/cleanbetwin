...
//        if (!currentUser() && $country==null) return redirect('/select_country');
        ...
        $super = $subscription->where('category', 'Super Plan')->get();
        $mega = $subscription->where('category', 'Mega Plan')->get();
        $investment = $subscription->where('category', 'Investment Scheme')->get();
        return view('vip', compact('super', 'mega', 'investment','sms'));
    }
