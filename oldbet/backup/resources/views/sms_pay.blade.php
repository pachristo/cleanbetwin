...
@section('title') {{$sub->category ?? 'Oops!! '}}@endsection
@section('vip', 'active')
...
@section('body')
        ...
        <div class="container" style="background: #fff; padding: 20px 15px; margin-bottom: 20px; margin-top: 20px;">
            <div class="row">
                <div class="col-sm-10 col-sm-offset-1">
                    @if($sub=='')
                    <blockquote>This Service is not available for <strong style="color: green;">{{currentUser()->country}}</strong>
                        <br><small>Not your country? <a href="{{route('/country')}}?sms">Change Here</a></small></blockquote>
                    ...
                    @endif
                    @else
                    <center><strong  style="color: red;">*{{$country ?? ''}}</strong></center>
    ...
    @if(!empty($sub))
    <script>
        // real=====pk_live_8f3ea6472dfe5bc566ccf8fdc87929a50a141f02
        var amount = Math.floor(({{$sub->nairaPrice}} * 100));
        var callback = "{{route('/smspaystack')}}";
        var subID = "{{$sub->id}}";
        ...
        function payWithPaystack(){
            var handler = PaystackPop.setup({
                ...
                amount: amount,
                ref: nowTim+'{{Str::slug($sub->category, '-')}}-{{Str::slug($sub->category, '-')}}-{{Str::slug($sub->planName, '-')}}-refID={{currentUser()->id}}',
       ...
       //real ======FLWPUBK-0fbbffad6dc4ee78782d1f0508c98c95-X
        ...
        function payWithRave() {
            var x = getpaidSetup({
                ...
                currency: cur,
                country: cou,
                // GH can also be passed as country, only country options to pass are NG and GH.
