...
$(document).ready(function () {
    ...
    $('#selectMailsForm').submit(function(e) {
        e.preventDefault();
        $('#selectMailsBtn').prop('disabled', true).html("Processing .....");
        ...
        $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#selectMailsForm')[0].reset();
                    $('#selectMailsInput').val(data.mails);
                    $('#selectMailsInput2').val(data.code);
                    swal("MAILS LOADED  SUCCESSFULLY", '', 'success');

                    $('#selectMailsBtn').prop('disabled', false).html("Load Mails");
    ...
    $('.unsubscribesmsUser').on('click', function (e) {
        e.preventDefault();
        var url = $(this).attr('href');
        var id = $(this).attr('data-id');
        var msg = $(this).attr('data-msg');
    ...
    $('#testimonialForm').submit(function (e) {
        e.preventDefault();
        $('#testimonialBtn').prop('disabled', true).html("Processing...");
        var url = $(this).attr('action');
        var dataString = new FormData($(this)[0]);
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status == 1) {
                    $('#testimonialBtn').prop('disabled', false).html("SUBMIT");
                    swal(data.encounters, '', 'warning');
                }
                else {
                    $('#testimonialForm')[0].reset();
                    swal("TESTIMONIAL UPLOADED SUCCESSFULLY", '', 'success');
                    $('#testimonialBtn').prop('disabled', false).html("SUBMIT");
                }
            },
            error: function (e, status) {
    ...
    $('.usersmsdetails').click(function () {
        var uid = $(this).attr('href');
        $('#smsebody').html('Fetching Data...');
        $.ajax({
            type: "GET",
            url: "usersmsdetails/" + uid,
            success: function (data) {
                $('#smsebody').html(data);
            },
            failure: function (data) {
    ...
    $('.testimonialDelete').click(function (e) {
        e.preventDefault();
        var url = $(this).attr('href');
        var id = $(this).attr('data-id');
        if (confirm("THIS TESTIMONIAL WILL BE DELETED PERMANENTLY") == true) {
            $.ajax({
                type: "GET",
                url: url + "/" + id,
                success: function (data) {
