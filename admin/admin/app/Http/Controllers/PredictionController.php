...
//        $theDates = Prediction::where('gameType',1)->where($key, 'Yes')->latest('gameDate')->distinct()->get(['gameDate']);
	...
	public function archives() {
		...
		if ($category == 'Super') {
			...
			$predict = Archive::where('creator', $id)->orderBy('id', 'DESC')->paginate(200);
			return view('/archive', ['allprediction' => $predict]);
		}
	}
...
//        $prediction->validateInput($request);
		...
		$game = array_add($data, 'creator', Auth::user()->id);
	...
	public function postNewBookingCode(Request $request, BookingCode $code) {
		...
		$check = $code->where('codeDate', $data['codeDate'])->where('VIPCategory', $data['VIPCategory'])->where('bookingCode', $data['bookingCode'])->first();
